...
const Profile = () => {
  ...
  const { data: projInfo } = getClientProjectInfo(projectId);
    ...
    <SafeAreaView className={`flex-1 ${styles.background}`}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View className="w-full justify-center items-center mt-6 mb-7 px-4">
          <TouchableOpacity className="flex items-center">
            ...
            <Text className={`text-2xl font-bold ${styles.textPrimary}`}>
              {projInfo?.clientFName} {projInfo?.clientLName}
            </Text>
        ...
        >
          ...
          {[
            { label: "Gender", value: projInfo?.sex },
            { label: "Address", value: projInfo?.clientAddress },
            { label: "Phone", value: projInfo?.clientContactNum },
            { label: "Project Status", value: projInfo?.status },
            { label: "System Type", value: projInfo?.systemType },
          ].map((item, index) => (
            <View
              key={index}
